...
function CompanySecurity (): JSX.Element {
  ...
  const [profile, setProfile] = useState<ProfileCompany>()
  ...
  const buttonValidation = async (): Promise<void> => {
    ...
    const response = await AuthApi.changeCompanyPassword(localStorage.getItem('jwtToken') as string, dto)
    if (response === '') {
  ...
  return (
    <div className='std-security'>
      ...
      <ModalValidation
        subject={profile?.name as string}
